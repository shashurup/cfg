...
def run_rofi(options):
    result = subprocess.run('rofi -dmenu -p i3',
                            shell=True,
                            text=True,
                            check=True,
                            capture_output=True,
...
result = subprocess.run(['i3', '--get-socketpath'], capture_output=True, check=True, text=True)
        ...
        if floating:
            ...
            if prop:
                ...
                if size < parent_size // 3:
                    ...
                    new_size = parent_size // 3
                run_command(sock, f'resize set {prop} {new_size}')
elif action == 'test':
    from pprint import pprint as pp
